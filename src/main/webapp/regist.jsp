...
<script>
	...
	function checkEmail() {
		...
		$.get("customerAction_findByEmail.action?email="+email+"&"+ new Date().getTime()  ,function(data,status){
			if(status=="success"){
				$("#span4").text(data);
			}
	...
	function checkUsername() {
		...
		// 3.打开连接
		xhr.open("GET",
				"${pageContext.request.contextPath}/customerAction_findByName.action?time="
	...
	function checkForm() {
		...
		if (password = null || password == "") {
			$("#span2").text("密码不能为空哦,傻吊");
			return false;
		}
		var password = $("#password").val();
		if (password.length < 6) {
			$("#span2").text("密码不能小于6位,傻吊");
			return false;
		}
		...
		var email = $("#email").val();
		if (email = null || email == "") {
			$("#span4").text("邮箱不能为空哦,傻吊");
			return false;
		}
		var re = /\w+@\w+\.\w+/g;
		var email = $("#email").val();
		if (!re.test(email)) {
			$("#span4").text("邮箱格式不正确,傻吊");
			return false;
		}
			...
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm" method="post" novalidate="novalidate"
						action="${ pageContext.request.contextPath }/customerAction_regist.action"
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="username" name="username"
										...
										<span id="span1"
										style="text-align:center; margin-top:20px; color:red"></span></td>
								...
								<tr>
									...
									<td><input id="rePassword" type="password"
										...
										autocomplete="off"> <span id="span3"
										style="text-align:center; margin-top:20px; color:red"></span></td>
								</tr>
								<tr>
